...
x_train_file = main_dir+f'\\files\\divided parts dataset\\x_train_part{part_number}.txt'
y_train_file = main_dir+f'\\files\\divided parts dataset\\y_train_part{part_number}.txt'
#x_train_file = 'C:/Users/tester/Desktop/Post-quantum_Authentication_FL/dataset/divided parts/x_train_part1.txt'
#y_train_file = 'C:/Users/tester/Desktop/Post-quantum_Authentication_FL/dataset/divided parts/y_train_part1.txt'
...
shuffled_dataset = TensorDataset(x_tensor[shuffled_indices], y_tensor[shuffled_indices])
 ...
 # divide the dateset into  sereral parts
...
for part_idx, part_dataset in enumerate(part_datasets):
    ...
    np.savetxt(f'C:/Users/tester/Desktop/Post-quantum_Authentication_FL/dataset/divided parts/x_train_part{part_idx+1}.txt', x_part.numpy())
    np.savetxt(f'C:/Users/tester/Desktop/Post-quantum_Authentication_FL/dataset/divided parts/y_train_part{part_idx+1}.txt', y_part, fmt='%d')
'''
...
def train(model, dataloader, criterion, input_size, num_classes, optimizer, device, epochs=20, print_every=10, validate_every=10):
    model.to(device)
    for epoch in range(epochs):
        ...
        for i, (inputs, labels) in enumerate(dataloader, 1):
            ...
            if i % validate_every == 0:
                validate(model, validation_dataloader, criterion, device)
...
train(model, train_dataloader, criterion, input_size, num_classes, optimizer, device='cpu', epochs=20)


torch.save(model.state_dict(), main_dir+f'\\files\\local_model{part_number}.pth')