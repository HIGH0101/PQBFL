...
def wrapfiles(model_data, signature_data):
    ...
    with tarfile.open(fileobj=tar_buffer, mode='w') as tar:
        ...
        model_info = tarfile.TarInfo(name='local_model1.pth')
...
# Load the smart contract ABI and address 
    ...
    # verification Signature
        Server_pubkey = open(main_dir + "/server/keys/pq_pub_key.txt",'rb').read()    # It is suppose the server's public key is received from a secure channel
        assert verify(Server_pubkey, Model,server_model_signature)

# Train_local_model
        
        Local_model= open(main_dir + "/client/files/local_model1.pth",'rb').read()   # training('primary_Model')   
